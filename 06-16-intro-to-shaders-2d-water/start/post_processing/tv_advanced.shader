...
// Other corners
  ...
  // x1 = x0 - i1 + 1.0 * C.xx ;
  // x2 = x0 - 1.0 + 2.0 * C.xx ;
  vec4 x12 = vec4(x0.xy, x0.xy) + vec4(C.xx, C.zz);
  x12.xy -= i1;
...
//
//	float red 	=   texture(	SCREEN_TEXTURE, 	vec2(uv.x + xOffset -0.01*rgbOffsetOpt,y)).r+staticVal;
//	float green = 	texture(	SCREEN_TEXTURE, 	vec2(uv.x + xOffset,	  y)).g+staticVal;
//	float blue 	=	texture(	SCREEN_TEXTURE, 	vec2(uv.x + xOffset +0.01*rgbOffsetOpt,y)).b+staticVal;
//
